<?php
 echo $_POST['user_signin'];
 echo $_POST['pass_signin'];
 exit();
// Kết nối database và thông tin chung
require_once 'models/DB.php';
 
// Nếu có tồn tại phương thức post từ ajax gưỉ
if (isset($_POST['user_signin']) && isset($_POST['pass_signin']))
{
    ...
    {
        $sql_check_user_exist = "SELECT username FROM user WHERE username = '$user_signin'";
        // Nếu tồn tại username
        if ($db->num_rows($sql_check_user_exist))
        {
            $pass_signin = md5($pass_signin);
            $sql_check_signin = "SELECT username, password FROM user WHERE username = '$user_signin' AND password = '$pass_signin'";
