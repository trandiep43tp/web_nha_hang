...
class pager
{
	...
	{
		$pages = (($count % $limit) == 0) ? $count / $limit : floor($count / $limit) + 1; //hàm floor làm tròn xuongs floor(3,2)=3
		return $pages;
	}
	...
	function pageList($curpage, $pages,$option="")
	{
		$page_list="";
			...
			{
				$page_list.='<span class="div_trang">'."<a href =\"".$_SERVER['PHP_SELF']."?page=1$option\" title=\"Trang đầu\"><<</a></span>";		//$_SERVER['PHP_SELF'] Trả về tên file của file đang được chạy.
			}
			if(($curpage-1)>0)
			{
	...
	function nextPrev($curpage,$pages)//chỉ hiển thị 2 nút Truoc và Sau
		{
			...
			{
				$next_prev.="<a href =\"".$_SERVER['PHP_SELF']."?page=".($curpage+1)."\">Đến trang sau</a>"; //$_SERVER['PHP_SELF'] Trả về tên file của file đang được chạy.
			}
			return $next_prev;
		}
}
